...
export default {
  ...
  runAction() {
    ...
    var fileName = editor.getFileName().split(".")[0];
    var dir = atom.project.getPaths();
    command = atom.project.getPaths() + '/run.sh';
    console.log(dir);
    console.log(command);
    args = [dir];

    stdout = (output) => console.log("OUT: " + output);
    stderr = (code) => console.log("Error: " + code);
    this.consoleManager.sendCommand(command, args, stdout, stderr);
  },
